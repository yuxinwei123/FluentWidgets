...
void FluentWindow::setWindowIcon(const QIcon& icon) {
...
void FluentWindow::setupUI() {
    ...
    connect(m_titleBar, &FluentTitleBar::themeToggleRequested, this, [](bool dark) {
        auto& mgr = FluentThemeManager::instance();
        mgr.setThemeMode(dark ? FluentThemeMode::Dark : FluentThemeMode::Light);
    });
